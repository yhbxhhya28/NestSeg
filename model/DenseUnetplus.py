...
class DenseUnetplus(nn.Module):
    def __init__(self, n_class):
        super(DenseUnetplus, self).__init__()
        self.num_layers = 161
        ...
        self.feature_decoder3 = Feature_decoder(in_feature=2*self.bt2, out_feature=self.bt2)

        self.feature_up2=Up_decoder(in_f=self.bt2, out_f=self.bt1)
        self.feature_decoder2 = Feature_decoder(in_feature=2*self.bt1, out_feature=self.bt1)

        self.feature_up1=Up_decoder(in_f=self.bt1, out_f=self.bt0)
        self.feature_decoder1 = Feature_decoder(in_feature=2*self.bt0, out_feature=self.bt0)

        self.feature_up0=Up_decoder(in_f=self.bt0, out_f=self.bt_initial)
        self.feature_decoder0 = Feature_decoder(in_feature=2*self.bt_initial, out_feature=self.bt_initial)
...
def unit_test():
    ...
    #print(DenseUnetplus(9))
    model =  DenseUnetplus(9).to(device)
