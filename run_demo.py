...
parser.add_argument('--weight_name', '-w', type=str, default='NestSeg')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            logits = model(images)  # logits.size(): mini_batch*num_class*480*640,in:iamges/labels n4hw,nhw
            #print(logits[0,:,0,0])
            end_time = time.time()
            ...
            prediction = logits.argmax(1).cpu().numpy().squeeze().flatten() #默认按行降维展开# prediction and label are both 1-d array, size: minibatch*640*480
            print(logits.argmax(1)[:,0,0])
